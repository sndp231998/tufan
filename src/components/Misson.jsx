...
import { LanguageContext, LanguageProvider } from "../themeContext";

const MissionIcon = () => (
  <svg
    className="w-16 h-16 text-[#E85C40]"
    viewBox="0 0 16 16"
    fill="currentColor"
    xmlns="http://www.w3.org/2000/svg"
  >
    <g id="SVGRepo_bgCarrier" storeWidth="0"></g>
  ...
  >
    ...
    <g id="SVGRepo_iconCarrier">
      ...
      <path
        ...
        stroke="currentColor"
      ></path>{" "}
      <circle cx="12" cy="12" r="2" stroke="background"></circle>{" "}
    </g>
  </svg>
);
...
const Card = ({ icon: Icon, title, items }) => (
  <div className="bg-white bg:text-foreground rounded-2xl p-8 shadow-sm flex flex-col items-center space-y-6 min-w-[300px] dark:bg-[#262728] dark:border-[#262728]">
    <Icon />
    ...
    <ul className="space-y-4 w-full">
      {items.map((item, index) => (
        <li key={index} className="flex items-center space-x-3">
          <ArrowIcon />
          <span className="text-gray-700 dark:text-foreground">{item}</span>
        </li>
      ))}
    </ul>
  </div>
...
const Misson = () => {
  const content = {
    Eng: {
      ...
      vision: {
        items: ["Shape the future of transportation", "Customer Empowerment"],
      },
      values: {
        items: ["Reliability", "Innovation", "Customer Satisfaction"],
      },
    },
    Nep: {
      mission: {
        items: [
          "आधुनिक प्रविधि",
          "सुरक्षा र सन्तुष्टि",
          "यात्रामा क्रान्ति ल्याउनुहोस्",
        ],
      },
      vision: {
        items: ["यातायातको भविष्य निर्माण", "ग्राहक सशक्तिकरण"],
      },
      values: {
        items: ["विश्वसनीयता", "नवाचार", "ग्राहक सन्तुष्टि"],
      },
    },
  };
  ...
  return (
    <div className="w-full max-w-7xl mx-auto p-3">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <Card
          icon={MissionIcon}
          title={language === "eng" ? "Mission" : "उद्देश्य"}
          items={
            language === "eng"
              ? content.Eng.mission.items
              : content.Nep.mission.items
          }
        />
        <Card
          icon={VisionIcon}
          title={language === "eng" ? "Vision" : "दृष्टिकोण"}
          items={
            language === "eng"
              ? content.Eng.vision.items
              : content.Nep.vision.items
          }
        />
        <Card
          icon={ValuesIcon}
          title={language === "eng" ? "Values" : "मूल्यहरू"}
          items={
            language === "eng"
              ? content.Eng.values.items
              : content.Nep.values.items
          }
        />
      </div>
    </div>
